...
export function TransactionHistory() {
  ...
  const ethQuery = trpc.getTransactions.useQuery(
    ...
    {
      enabled: !!address && (!selectedNetwork || selectedNetwork === sepolia.id),
  ...
  const arbQuery = trpc.getTransactions.useQuery(
    ...
    {
      enabled: !!address && (!selectedNetwork || selectedNetwork === arbitrumSepolia.id),
      refetchInterval: 60000,
      staleTime: 30000,
    }
  );
  ...
  const opQuery = trpc.getTransactions.useQuery(
    ...
    {
      enabled: !!address && (!selectedNetwork || selectedNetwork === optimismSepolia.id),
      refetchInterval: 60000,
      staleTime: 30000,
    }
  );
  ...
  const transactionQueries = selectedNetwork
    ? selectedNetwork === sepolia.id ? [ethQuery]
      : selectedNetwork === arbitrumSepolia.id ? [arbQuery]
        : selectedNetwork === optimismSepolia.id ? [opQuery]
          : []
  ...
  const getExplorerLink = (hash: string, chainId: number, type: "tx" | "address") => {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">
              {selectedNetwork ? `${getNetworkInfo(selectedNetwork)?.name || 'Selected network'}` : 'All networks'} • Last {limit} transactions
            </p>
          </div>
          {isRefetching && (
            ...
            {shouldUseVirtualization && (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="rounded"
                />
                <label htmlFor="virtualization-toggle" className="text-muted-foreground">
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            Array.from({ length: 3 }, (_, i) => (
              <div key={`transaction-skeleton-${i + 1}`}>{renderTransactionSkeleton()}</div>
            ))
          ) : hasError ? (
            ...
            <div className="py-8 text-center">
              ...
              <p className="mt-1 text-muted-foreground text-sm">
                {address ? 'Try connecting to a different network or making some transactions' : 'Connect your wallet to view transactions'}
              </p>
            </div>
          ) : (
